...
def internal_fetch_sheet_data(client, conf, tab):
    try:
        ...
        COMPANY_KEYS, POSITION_KEYS, STAGE_KEYS = ["Company", "Client", "Cliente", "公司", "客户"], ["Position", "Role", "职位"], ["Stage", "Status", "阶段"]
        ...
        for r in rows:
            if not r: continue
            fc = r[0].strip()
            if fc in COMPANY_KEYS: details.extend(flush(block)); block = {"c": r[1] if len(r) > 1 else "Unk", "p": "Unk", "cands": {}}
    ...
    try:
        ...
        for row in rows:
            ...
            if found_header:
                ...
                matched = "Unknown"; c_norm = normalize_text(consultant_name)
...
def load_data_from_api(client, quanbu):
    ...
    return {"team_data": team_data, "rec_stats": rec_stats_df, "rec_details": rec_details_df, "rec_hist": pd.DataFrame(), "sales_all": all_sales_df, "last_updated": datetime.now().strftime("%H:%M:%S")}
...
def main():
    ...
    sales_df_2q = all_sales_df[all_sales_df['Quarter'].isin([CURRENT_Q_STR, PREV_Q_STR])].copy() if not all_sales_df.empty else pd.DataFrame()
    ...
    with tab_dash:
        ...
        if not rec_stats_df.empty:
            ...
            rec_summary[['Role', 'CV Target']] = rec_summary['Consultant'].apply(lambda x: pd.Series(get_role_target(x)))
            ...
            st.dataframe(rec_summary, use_container_width=True, hide_index=True, column_config={"Activity %": st.column_config.ProgressColumn("Activity %", format="%.0f%%", min_value=0, max_value=100)})
        ...
        for conf in dynamic_team_config:
            ...
            c_sales = sales_df_2q[sales_df_2q['Consultant'] == c_name].copy() if not sales_df_2q.empty else pd.DataFrame()
            ...
            sent_curr = rec_stats_df[(rec_stats_df['Consultant'] == c_name) & (rec_stats_df['Month'].isin(curr_q_months))]['Sent'].sum()
            sent_hist = rec_stats_df[(rec_stats_df['Consultant'] == c_name) & (rec_stats_df['Month'].isin(prev_q_months))]['Sent'].sum()
                    ...
                    for q_name in [PREV_Q_STR, CURRENT_Q_STR]:
                        ...
                        for idx, row in q_data.iterrows():
                            ...
                            if target_is_met and row['Status'] == 'Paid' and level > 0:
                                comm = calculate_single_deal_commission(row['Candidate Salary'], multiplier) * row['Percentage']
                                ...
                                if p_date:
                                    ...
                                    if p_date <= datetime.now() + timedelta(days=20):
                                        if q_name == CURRENT_Q_STR: total_comm_curr += comm
                                        else: total_comm_hist += comm
            ...
            if not is_intern and is_team_lead and is_target_met_curr and not sales_df_2q.empty:
                ov_mask = (sales_df_2q['Status'] == 'Paid') & (sales_df_2q['Consultant'] != c_name) & (sales_df_2q['Consultant'] != "Estela Peng")
                for _, row in sales_df_2q[ov_mask].iterrows():
                    ...
                    if p_date and p_date <= datetime.now() + timedelta(days=20):
                        ...
                        team_lead_overrides.append({"Leader": c_name, "Source": row['Consultant'], "Salary": row['Candidate Salary'], "Percentage": row['Percentage'], "Date": p_date.strftime("%Y-%m-%d"), "Bonus": bonus})

            financial_curr.append({"Consultant": c_name, "Role": role, "GP Target": gp_target, "Paid GP": c_sales_curr['GP'].sum() if not c_sales_curr.empty else 0, "Fin %": fin_curr, "Status": status_text_curr, "Est. Commission": total_comm_curr})
            financial_hist.append({"Consultant": c_name, "Role": role, "GP Target": gp_target, "Paid GP": c_sales_hist['GP'].sum() if not c_sales_hist.empty else 0, "Fin %": fin_hist, "Status": status_text_hist, "Est. Commission": total_comm_hist})
        ...
        st.dataframe(pd.DataFrame(financial_curr).sort_values('Paid GP', ascending=False), use_container_width=True, hide_index=True, column_config={"Est. Commission": st.column_config.NumberColumn("Payable Comm.", format="$%d"), "Fin %": st.column_config.ProgressColumn("Financial % (Booked)", format="%.0f%%", min_value=0, max_value=100)})
                ...
                if not final_sales_df.empty:
                    ...
                    for q_name in [PREV_Q_STR, CURRENT_Q_STR]:
                        ...
                        if not q_data.empty:
                            ...
                            st.dataframe(q_data[['Onboard Date Str', 'Payment Date', 'Commission Day', 'Candidate Salary', 'GP', 'Status', 'Applied Level', 'Final Comm']], use_container_width=True, hide_index=True)
