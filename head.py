...
def load_data_from_api(client, quanbu):
    ...
    time.sleep(1)
    rec_hist_df = fetch_historical_recruitment_stats(client, exclude_months = quanbu)
    time.sleep(1)
...
#   start_m, end_m = 10, 12
#   quanbu = [f"{CURRENT_YEAR}{m:02d}" for m in range(start_m, end_m + 1)]
    ...
    with tab_dash:
        ...
        for conf in dynamic_team_config:
            ...
            if not is_intern:
                ...
                def normalize_text(text):
                    return ''.join(
                        c for c in unicodedata.normalize('NFD', str(text)) if unicodedata.category(c) != 'Mn').lower()
                    ...
                    try:
                        ...
                        def flush(b):
                            ...
                            for _, c_data in b['cands'].items():
                                ...
                                stat = "Offered" if is_off else ("Interviewed" if is_int else "Sent")
                                res.append(
                                    {"Consultant": conf['name'], "Month": tab, "Company": b['c'], "Position": b['p'],
                                     "Status": stat,
                                     "Count": 1})
                            return res
                        ...
                        for row in rows:
                            ...
                            if not found_header:
                                if any("linkeazi" in c and "consultant" in c for c in row_lower) and any(
                                    ...
                                    continue
                            if found_header:
                                if "POSITION" in " ".join(row_lower).upper() and "PLACED" not in " ".join(
                                    row_lower).upper(): break
                                if len(row) <= max(col_cons, col_onboard, col_sal): continue
                                consultant_name = row[col_cons].strip()
                                if not consultant_name: continue
                                onboard_date = None
                                ...
                                try:
                                    salary = float(
                                        str(row[col_sal]).replace(',', '').replace('$', '').replace('MXN', '').replace(
                                            'CNY',
                                            '').strip())
                                except:
                                    salary = 0
                                ...
                                sales_records.append({
                                    ...
                                    "Percentage": pct_val,
                                    "Onboard Date": onboard_date, "Onboard Date Str": onboard_date.strftime("%Y-%m-%d"),
                                    "Payment Date": row[col_pay].strip() if col_pay != -1 and len(
                                        row) > col_pay else "",
                                    "Payment Date Obj": pay_date_obj, "Status": status,
                ...
                def main():
                    ...
                    cache = st.session_state['data_cache']
                    dynamic_team_config = cache['team_data']
                    rec_stats_df, rec_details_df, rec_hist_df, all_sales_df = cache['rec_stats'], cache['rec_details'], \
                    cache[
                        'rec_hist'], cache['sales_all']
                    st.caption(f"📅 Snapshot: {cache['last_updated']}")
                    ...
                    with tab_dash:
                        ...
                        if not rec_stats_df.empty:
                            ...
                            rec_summary = rec_stats_current.groupby('Consultant')[
                                ['Sent', 'Int', 'Off']].sum().reset_index()
                        ...
                        with st.expander("📜 Historical Recruitment Data"):
                            ...
                            if not rec_stats_prev.empty:
                                # 汇总上季度数据
                                summary_prev = rec_stats_prev.groupby('Consultant')[
                                    ['Sent', 'Int', 'Off']].sum().reset_index()
                                st.dataframe(
                                    ...
                                    if not paid_sales.empty:
                                        ...
                                        for month_key in unique_months:
                                            ...
                                            if level > 0:
                                                ...
                                                for idx in pending_indices:
                                                    row = paid_sales.loc[idx]
                                                    deal_comm = calculate_single_deal_commission(
                                                        row['Candidate Salary'], multiplier) * \
                                                                row['Percentage']
                                                    paid_sales.at[idx, 'Applied Level'] = level
                                                    paid_sales.at[idx, 'Commission Day Obj'] = payout_date
                                                    # 如果未达标，计算出的 comm 暂时保留在 DataFrame 以便 debug，但在总数 total_comm 中不加
                                        ...
                                        for idx, row in paid_sales.iterrows():
                                            comm_date = row['Commission Day Obj']
                                            if is_target_met:  # 关键判断：是否达标
                                                if pd.notnull(comm_date) and comm_date <= datetime.now() + timedelta(
                                                        days=20):
                                                    total_comm += row['Final Comm']
                                            else:
                                                # 未达标，佣金归零
                                    ...
                                    if not final_sales_df.empty:
                                        ...
                                        for q_name in [CURRENT_Q_STR, PREV_Q_STR]:
                                            ...
                                            if not q_data.empty:
                                                ...
                                                q_data['Pct Display'] = q_data['Percentage'].apply(
                                                    lambda x: f"{x * 100:.0f}%")

                                                st.dataframe(q_data[
                                                                 ['Onboard Date Str', 'Payment Date', 'Commission Day',
                                                                  'Candidate Salary',
                                                                  'Pct Display', 'GP', 'Status', 'Applied Level',
                                                                  'Final Comm']],
                                                             use_container_width=True, hide_index=True,
                                                             column_config={
                                                                 "Commission Day": st.column_config.TextColumn(
                                                                     "Comm. Date"),
